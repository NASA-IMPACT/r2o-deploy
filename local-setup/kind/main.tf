...
resource "null_resource" "setup-kind" {
  ...
  provisioner "local-exec" {
    ...
    "KIND_EXPERIMENTAL_PROVIDER=podman && systemd-run --scope --user -p \"Delegate=yes\" kind create cluster --name ${var.cluster_name} --config=config.yaml && kind export kubeconfig --name ${var.cluster_name} --kubeconfig=/home/opkind/kubeconfig"
    ) : (
    # Default to docker
